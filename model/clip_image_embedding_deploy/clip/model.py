...
class Clip():
    ...
    def generate_caption(self, pil_image: Image) -> str:
        ...
        inputs = self.caption_processor(images=pil_image, return_tensors="pt", padding=True).to(self.device)
        inputs = inputs.to(self.dtype)

        with torch.no_grad():
            image_tensor = self.caption_model.get_image_features(**inputs)
        
        image_features = {'image_embedding':image_tensor.tolist()}
...
def handle(inputs: Input) -> Optional[Output]:
    ...
    probs = _service.generate_caption(input_image)
